...
NimModel <- nimbleCode({
  ...
  mean.pD ~ dunif(0, 1)
  for (k in 1:2){
    alphaD[k] ~ dnorm(0, 0.01)
  }
  beta0D ~ dnorm(0, 0.01)
  for (k in 1:3){
    betaD[k] ~ dnorm(0, 0.01)
  }
  # Likelihood
  ...
  for (k in 1:2){
    alphaI[k] ~ dnorm(0, 0.01)
  }
  beta0I ~ dnorm(0, 0.01)
  for (k in 1:3){
    betaI[k] ~ dnorm(0, 0.01)
  }
  gamma0DI ~ dnorm(0, 0.01)
  # gamma1 ~ dnorm(0, 0.2)
  ...
  for (k in 1:2){
    alphaS[k] ~ dnorm(0, 0.01)
  }
  beta0S ~ dnorm(0, 0.01)
  for (k in 1:3){
    betaS[k] ~ dnorm(0, 0.01)
  }
  gamma0IS ~ dnorm(0, 0.01)
  gamma0DS ~ dnorm(0, 0.01)
  # gamma1 ~ dnorm(0, 0.2)
  # epsilon0 ~ dnorm(0, 0.2)
...
setwd('C:/Users/jpt93/Documents/R/Occu-abu model/new_simulator/three_species/M=600/DI=-1_IS=-1_DS=1, pD=0.5, pI = 0.5, pS = 0.5')
`%!in%` = Negate(`%in%`)
...
files = list.files('C:/Users/jpt93/Documents/R/Occu-abu model/new_simulator/three_species/M=600/DI=-1_IS=-1_DS=1, pD=0.5, pI = 0.5, pS = 0.5')
