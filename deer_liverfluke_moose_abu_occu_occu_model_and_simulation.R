...
data.fn <- function(J = 600, K = 4, mean.lambda.D = 0.5, beta1.D = 1, N.days = 7,
  ...
  N.D <- rpois(n = J, lambda = lambda.D)      # Realised abundance
  ...
  for (i in 1:K){                         # Generate counts by survey
    C.D[,i] <- rbinom(n = J, size = N.days, prob = p.D[,i])
  }
  ...
  for (i in 1:K){                         # Generate counts by survey
    C.I[,i] <- rbinom(n = J, size = Z.I, prob = p.I[,i])
  }
  sum.C.I <- sum(C.I)
  ...
  for (i in 1:K){                         # Generate counts by survey
    C.s[,i] <- rbinom(n = J, size = Z.s, prob = p.s[,i])
  }
  sum.C.s <- sum(C.s)
