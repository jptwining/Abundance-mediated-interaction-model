...
data.fn <- function(M = 1000, J = 4, mean.lambda.D = 1, beta1.D = 1, N.days = 7,
...
NimModel <- nimbleCode({
  ...
  for (j in 1:J){
    zS[j] ~ dbern(psiB[j])
    logit(psiB[j]) <- beta0S + betaS[1] * forestS[j] + betaS[2] * roadS[j] + betaS[3] * agricultureS[j] +gamma0 *nD[j] + gamma1*forestS[j]*nD[j]
...
parameters <-  c("alpha0D", "alphaD", "beta0D", "betaD", "alpha0S", "alphaS", "beta0S", "betaS", "totalZs","gamma0", "gamma1", "totalnD")
...
#inits
Niminits=list(beta0D=0,nD=apply(yD,1,max),zS=apply(yS,1,max),alphaD=c(0,0),betaD=c(0,0,0),alphaS=c(0,0),betaS=c(0,0,0),gamma0=0, gamma1=0)
...
dir.create(paste0(path,"/JoshSim_OccuAbu/M=1000/g0=-1_g1=0"))
...
out = foreach(rep=1:simrep) %dopar% {
  setwd(paste0(path,"/JoshSim_OccuAbu/M=1000/g0=-1_g1=0"))
  files = list.files(paste0(path,"/JoshSim_OccuAbu/M=1000/g0=-1_g1=0"))
