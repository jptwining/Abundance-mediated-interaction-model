...
data.fn <- function(I = 600, J = 4,  mean.psi.D = 0.5, beta1.D = 1, 
                    beta2.D = -1, beta3.D = 1, mean.detection.D = 0.5, alpha1.D = -1, alpha2.D = 0.15,
                    mean.lambda.I = 0.5, beta1.I = 1, K = 7,
  ...
  for (i in 1:J){                         # Generate counts by survey
    C.D[,i] <- rbinom(n = I, size = Z.D, prob = p.D[,i])
  }
  sum.C.D <- sum(C.D)
...
NimModel <- nimbleCode({
  ...
  for (i in 1:I) {
    zD[i] ~ dbern(psiD[i])
    logit(psiD[i]) <-  beta0D + betaD[1] * forestD[i] + betaD[2] * roadD[i] + betaD[3] * agricultureD[i] 
  }
  #Observation model for replicated counts
  for (i in 1:I) {
    for (j in 1:nsurveys){
      logit(pD[i,j]) <- alpha0D + alphaD[1] * DATE[i,j] + alphaD[2] * occ[i,j]
      yD.new[i,j] ~ dbern(pD[i,j] * zD[i])
    }
  }
  # --- Priors ---
  ...
  for (i in 1:I) {
    ...
    log(lambdaI[i]) <-  beta0I + betaI[1] * forestI[i] + betaI[2] * roadI[i] + betaI[3] * agricultureI[i] + gamma0DI * zD[i]
  }
  ...
  for (i in 1:I) {
    ...
    log(lambdaS[i]) <-  beta0S + betaS[1] * forestS[i] + betaS[2] * roadS[i] + betaS[3] * agricultureS[i] + gamma0IS * nI[i] + gamma0DS * zD[i]
  }
