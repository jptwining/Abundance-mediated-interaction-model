...
NimModel <- nimbleCode({
  ...
  for (t in 1:nyears) {
    for (j in 1:J[t]) {
      ...
      log(lambda[j,t]) <-  beta0D[t] + betaD[1] * deerD[j,t] + betaD[2] * edgeD[j,t] 
    }
  }
  ...
  for (t in 1:nyears) {
    for (j in 1:J[t]) {
      for (k in 1:nsurveys){
        logit(pD[j,k,t]) <- alpha0D[t] + alphaD[1] *  occ[j,k,t] + alphaD[2] * DATE[j, k, t] +  alphaD[3] * pow(DATE[j,k,t], 2) 
        pD.cum[j,k,t] <- (1 - (1 - pD[j,k,t])^nD[j,t]) * K2D[j,k,t] #prob detecting at least 1 ind per day
  ...
  for (t in 1:nyears) {
    for (j in 1:J[t]) {
      for (k in 1:nsurveys){
        logit(pI[j,k,t]) <- alpha0I[t] + alphaI[1] *  occ[j,k,t] + alphaI[2] * temp[j,k,t] + alphaI[3] * DATE[j, k, t] +  alphaI[4] * pow(DATE[j,k,t], 2) 
        pI.cum[j,k,t] <- (1 - (1 - pI[j,k,t])^nI[j,t])* K2D[j,k,t]  #prob detecting at least 1 ind per day
  ...
  for (t in 1:nyears) {
    beta0S[t] ~ dnorm(0, 0.1) #use fixed effects
  }
  for (t in 1:nyears) {
    alpha0S[t] ~ dnorm(0, 0.1) #use fixed effects
  }
  for (k in 1:4) {
  ...
  # State model for abundance for subordinate species (S)
  # Likelihood
  for (t in 1:nyears) {
    for (j in 1:J[t]) {
      nS[j,t] ~ dpois(lambdaS[j,t])
      log(lambdaS[j,t]) <- beta0S[t] + betaS[1] * conifmixedS[j,t] + betaS[2] * snowdepthS[j,t] + betaS[3] * deciduousS[j,t] + gamma0DS * nD[j,t] + gamma0IS * nI[j, t]
    }
  }
  ...
  for (t in 1:nyears) {
    for (j in 1:J[t]) {
      for (k in 1:nsurveys){
        logit(pS[j,k,t]) <- alpha0S[t] + alphaS[1] *  occ[j,k,t] +  alphaS[2] * DATE[j, k, t] +  alphaS[3] * pow(DATE[j,k,t], 2) + alphaS[4] * deer[j,k,t]
        pS.cum[j,k,t] <- (1 - (1 - pS[j,k,t])^nS[j,t]) * K2D[j,k,t] #prob detecting at least 1 ind per day
        yS.new[j,k,t] ~ dbinom(pS.cum[j,k,t],size=D)
      }
    }
  }
  ...
  #   for (j in 1:J[t]){
  #     for (k in 1:nsurveys) {
  #       YsimS[j,k,t] ~  dpois(pS[j,k,t] * nS[j,t])
  #       YexpS[j,k,t] <- pS[j,k,t] * nS[j,t]
  # 
  #       #components for test 1
