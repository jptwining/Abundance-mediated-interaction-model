...
data.fn <- function(I = 600, J = 4, mean.lambda.D = 0.5, beta1.D = 1, N.days = 7,
  ...
  C.s <- matrix(NA, nrow = I, ncol = K)     # Prepare matrix for counts
  for (i in 1:J){                         # Generate counts by survey
...
zI.init=1*(apply(Nimdata$yI,c(1),sum)>0)
zS.init=1*(apply(Nimdata$yS,c(1),sum)>0)


Niminits=list(beta0D=0,nD=nD.init,zS=zS.init,zI=zI.init,alphaD=c(0,0),betaD=c(0,0,0), beta0I=0,alphaI=c(0,0),betaI=c(0,0,0),alphaS=c(0,0),betaS=c(0,0,0),gamma0DI=0, gamma0IS=0)
