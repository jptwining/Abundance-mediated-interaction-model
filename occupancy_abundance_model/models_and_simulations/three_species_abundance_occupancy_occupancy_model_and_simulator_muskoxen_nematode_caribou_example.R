...
data.fn <- function(I = 600, J = 4, mean.lambda.D = 1, beta1.D = 1, K = 7,
                    beta2.D = -1, beta3.D = 1, mean.individual.detection.D = 0.5, alpha1.D = -1, alpha2.D = 0.15,
                    mean.psi.I = 0.5, beta1.I = 1, 
                    beta2.I = -1, beta3.I = 1, mean.detection.I = 0.5, alpha1.I = -1, alpha2.I = 0.15,
                    mean.psi.s = 0.5, beta1.s = 1, beta2.s = -1, beta3.s = 1, mean.detection.s = 0.5,
                    alpha1.s = -1, alpha2.s = 0.15, gamma0DI = 1, gamma0IS = -1){
  #
  ...
  beta0.I <- qlogis(mean.psi.I)            # Mean abundance on link scale
  psi.I <- plogis(beta0.I + beta1.I*forest + beta2.I*road + beta3.I*agr +gamma0DI*N.D)
  Z.I <- rbinom(I, 1, psi.I)
  Z.total.I <- sum(Z.I)                    # True occupancy in sample
  
  
  # Model for  INTERMEDIATE sp. observations
  alpha0.I <- qlogis(mean.detection.I)        # mean detection on link scale
  p.I <- plogis(alpha0.I + alpha1.I*day + alpha2.I*occ)
  C.I <- matrix(NA, nrow = I, ncol = J)     # Prepare matrix for counts
  for (i in 1:J){                         # Generate counts by survey
    C.I[,i] <- rbinom(n = I, size = Z.I, prob = p.I[,i])
  }
  sum.C.I <- sum(C.I)
  psi.obs.I <- mean(apply(C.I,1,max)>0)   
  
  #  Model for SUBORDINATE sp. abundance
  beta0.s <- qlogis(mean.psi.s)               # Mean occupancy on logit scale
  psi.s <- plogis(beta0.s + beta1.s*forest + beta2.s*road + beta3.s*agr+ gamma0IS*Z.I)
  ...
  return(list(I = I, 
              ...
              summaxC.D = summaxC.D,
              psi.obs.D = psi.obs.D, 
              mean.psi.s  = mean.psi.s,
              beta0.s = beta0.s, 
              beta1.s = beta1.s,
              beta2.s = beta2.s,
              beta3.s = beta3.s,
              mean.detection.s = mean.detection.s,
              alpha0.s = alpha0.s,
              alpha1.s = alpha1.s, 
              alpha2.s = alpha2.s, 
              psi.s = psi.s, 
              mean.psi.I  = mean.psi.I,
              beta0.I = beta0.I,
              beta1.I = beta1.I,
              beta2.I = beta2.I, 
              beta3.I = beta3.I,
              mean.detection.I = mean.detection.I, 
              alpha0.I = alpha0.I,
              alpha1.I = alpha1.I,
              alpha2.I = alpha2.I, 
              psi.I = psi.I, gamma0DI = gamma0DI,
              gamma0IS = gamma0IS,
              Z.s = Z.s, p.s = p.s, 
              C.s = C.s, Z.total.s = Z.total.s,
              Z.I = Z.I, 
              p.I = p.I,
              C.I = C.I, 
              Z.total.I = Z.total.I,
...
#pull out count data for the three species
yD <- data$C.D
yI <- data$C.I
ys <- data$C.s
...
NimModel <- nimbleCode({
  ...
  for (i in 1:I) {
    zI[i] ~ dbern(psiI[i])
    logit(psiI[i]) <- beta0I + betaI[1] * forestI[i] + betaI[2] * roadI[i] + betaI[3] * agricultureI[i] + gamma0DI * nD[i]
  }
  
  #Observation model for replicated counts
    for (i in 1:I) {
      for (j in 1:nsurveys){
        yI.new[i, j] ~ dbern(pI[i, j] * zI[i])
        logit(pI[i, j]) <-alpha0I + alphaI[1] * DATE[i, j] + alphaI[2] *  occ[i, j]
      }
    }
  # --- Priors ---
  #Model for dominant species (S)
  #Priors for intermediate species (S)
  ...
  # Likelihood
  # State model for true abundance of dominant species (S)
  for (i in 1:I) {
    zS[i] ~ dbern(psiS[i])
    logit(psiS[i]) <- beta0S + betaS[1] * forestS[i] + betaS[2] * roadS[i] + betaS[3] * agricultureS[i] + gamma0IS * zI[i]
  }
...
#inits
Niminits=list(beta0D=0,beta0S=0, beta0I = 0, nD=apply(yD,1,max),zS=apply(yS,1,max),zI=apply(yI,1,max), alpha0D=0, alpha0I = 0, alpha0S=0, 
              betaD=c(0,0,0), betaI=c(0,0,0),betaS=c(0,0,0), alphaD=c(0,0),alphaI=c(0,0), alphaS=c(0,0), gamma0DI=0, gamma0IS=0)
