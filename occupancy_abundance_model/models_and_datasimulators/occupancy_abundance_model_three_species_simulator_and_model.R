...
#inits
Niminits=list(beta0D=0, beta0I=0, alpha0S=0, nD=apply(yD,1,max),zS=apply(yS,1,max),nI=apply(yI,1,max),alpha0D =0, alpha0I=0, alpha0S=0,
              betaD=c(0,0,0), betaI=c(0,0,0),betaS=c(0,0,0),alphaD=c(0,0), alphaI=c(0,0),alphaS=c(0,0),gamma0DI=0, gamma0IS=0, gamma0DS=0)
  ...
  if(paste("Occu_Abu_sim",rep, ".RData", sep="") %!in% files){
    ...
    for(chain in 1:n.chains){
      ...
      NimModel <- nimbleCode({
        ...
        for (i in 1:I){
          zS[i] ~ dbern(psiB[i])
          logit(psiB[i]) <- beta0S + betaS[1] * forestS[i] + betaS[2] * roadS[i] + betaS[3] * agricultureS[i] +gamma0IS *nI[i] 
          #Observation model for replicated cout
          for (j in 1:nsurveys){
            yS.new[i,j] ~ dbern(pS[i,j] * zS[i])
            logit(pS[i,j]) <- alpha0S + alphaS[1] * DATE[i,j] + alphaS[2] * occ[i,j]
